          ...
          {SERVICES.map((s) => (
            <div key={s.title} className="group relative rounded-3xl glass p-8 transition-all hover:-translate-y-1 hover:shadow-glow">
              ...
              <div className="mt-8 flex items-center gap-6">
                {s.slug && (
                  <Link to={`/services/${s.slug}`} className="inline-flex items-center gap-1 text-sm font-medium text-white hover:text-accent transition-colors">
                    Learn more <ArrowRight className="h-4 w-4" />
                  </Link>
                )}
                <Link to="/contact" className="inline-flex items-center gap-1 text-sm font-medium text-accent">
